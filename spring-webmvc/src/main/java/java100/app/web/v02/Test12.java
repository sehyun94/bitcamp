...
public class Test12 {
    ...
    @RequestMapping("m1")
    public String m1(
            String name, int age, boolean working, float height) {
    ...
    // 기본 타입외에 다른 타입으로 형변환을 요구하면 프론트 컨트롤러 오류 발생.
    //해결방안?
    // => String을 java.util.Date 객체로 변환시켜주는 변환기를 만들어 
    //    스프링에 꼽아야한다 
    // 변환기를 꼽는 방법
    // 1) 해당 페이지 컨트롤러에 @InitBinder를 꼽는다.
    // 
    ...
    // => 프론트 컨트롤러(DispatcherServlet) 라이언트로 부터 요청이 들어오면 
    //    클라이언트가 보낸 데이터를  적절하게 변환하기 위해 변환기를 셋팅한다.
    //    이것을 "Web Data Binding" 이라 표현한다. 
    //    문자열을 다른 타입의 값으로  변환시키는 객체를 "Web Data Binder" 라부른다
    // => 스프링은 문자열을 primitive 타입으로 바꿀수 있도록 기본 변환기를 제공한다. 
    //    그 외에 타입은 개발자가 만들어 셋팅해야 한다.
    // => @InitBinder가 붙은 메서드는 
    //    프론트 컨트롤러가 문자열을 특정 타입의 값으로 바꾸기 전에
    //    WebDataBinder를 추가로 설정하기 위해 호출하는 메서드이다.
    // => 그래서 요청이 들어올 때마다 매번 호출된다.
    //    즉 요청을 처리하기 전에 문자열을 다른 타입으로 바꿔주는 변환기를
    //    먼저 설치한 후 요청 핸들러를 호출하는 것이다.
    ...
    protected void initBinder(WebDataBinder binder) {
        System.out.println("Test12initBinder() 호출");
