...
public class Test02 {
    ...
    public String m2(HttpServletResponse response) { 
        // 콘텐츠를 출력할 때 기본 ISO-8859-1 character set 인코딩하기 때문에  
        // 한글을 출력하면 바꿀수 없다는 의미로 ? 로 출력된다.
        
        // 리턴하기 전에 콘텐츠 타입의 문자집합을 설정하면 될까?
        response.setContentType("text/plain;charset=UTF-8"); // 결과는 안된다.
        
        // 직접 헤더를 설정하면 될까?
        response.setHeader("Content-Type", "text/plain;charset=UTF-8"); // 결과는 안된다!
    ...
    //    또한 그 데이터가 UTF-8로 인코딩 되길 원할 때 이 메서드가 호출된다.
    // => 그런데 프론트 컨트롤러가 리턴 값을 출력할 때 이 조건을 분석하여 
    ...
    @RequestMapping("/m4")
    //@ResponseBody // HttpEntity, ResponseEntity 객체를 리턴할 때는 프론트 컨트롤러가
                    // 콘텐츠임을 알 수 있기 때문에 @ResponseBody를 붙일 필요가 없다.
