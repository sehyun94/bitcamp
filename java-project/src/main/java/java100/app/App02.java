...
/*
 * ver 17 : 자바에서 제공하는 컬렉션 API 사용하라 public class App { static boolean
 * confirm(String message) { Scanner keyScan = new Scanner(System.in);
 ...
 * 
 * //제네릭이 적용된 ArrayList를 사용 ArrayList<Score> list = new ArrayList<>();
 * 
 * while (true) { Score score = new Score(); // 성적 데이터 저장할 빈 객체를 준비.
 * score.input(); // 사용자로부터 입력받은 데이터를 인스턴스에 저장
 * 
 * // 인스턴스(주소)를 배열에 저장 list.add(score);
 * 
 * if (!confirm("계속하시겠습니까? ")) break; } Iterator<Score> iterator =
 * list.iterator();
 * 
 * while(iterator.hasNext()) { iterator.next().print();
 * 
 * } for (int i = 0; i < list.size(); i++) { // ((Score)list.get(i)).print();
 * list.get(i).print(); } } }
 */
/*
 * ##ver 16 // - ArrayList에 generic을 적용하여 저장하는 타입을 지정할 수 있게 한다. public class App
 * { static boolean confirm(String message) { Scanner keyScan = new
...
/*
 *
 * ver 15 // ScoreDao를 다른 타입에 대해서도 처리할 수 있도록 변경하라!
 * 
 * public class App { static boolean confirm(String message) { Scanner keyScan =
 * new Scanner(System.in); System.out.print(message); String response =
 * keyScan.nextLine().toLowerCase(); if (response.equals("y") ||
 * response.equals("yes")) return true; return false; }
 * 
 * public static void main(String[] args) {
 ...
 * list.size(); i++) { //list.get(i).print();
 * 
 * //Score s = (Score)list.get(i); //s.print();
 * 
 * ((Score)list.get(i)).print();
 * 
 * } } }
