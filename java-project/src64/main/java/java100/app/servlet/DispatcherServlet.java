...
public class DispatcherServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        //클라이언트가 요청한 URL에서 .do 를 제외한 페이지 컨트롤러 경로를 추출한다.
        ...
        // 페이지 컨트롤러 객체서 @RequestMapping이 붙은 메서드를 찾는다.
        ...
        for (int i = 0; i < params.length; i++) {
            if (params[i].getType() == HttpServletRequest.class) {
                ...
                if (anno != null) { 
                    ...
                    // RequestParam 애노테이션이 붙을때 
                    values[i] = toParamTypeValue(params[i], value);
                } else { 
                    
                    //RequestParam 애노테이션이 붙지 않았을 때 
    ...
    private Object toParamTypeObject(Parameter parameter, HttpServletRequest request
        ...
        // 객체를 정상적으로 생성했다면 타입의 셋터 메서드를 추출한다.
        @SuppressWarnings("unchecked")
        ...
        for (Method setter : setters) {
         ...
         // String 값을 바꾸지 못했으면 셋터가 원하는 타입의 값으로 바꾸지 못했으면 셋터를 호출하지 않는다.
