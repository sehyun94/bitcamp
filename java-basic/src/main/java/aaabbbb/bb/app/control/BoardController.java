...
public class BoardController extends GenericController<Board> {
    ...
    @Override
    public void destory() {
        try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(this.FilePath)))) {

            for (Board board : this.list) {
                out.print(board.toCSVString());
            }
            out.flush();
